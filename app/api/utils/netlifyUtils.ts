...
// Dynamically import mammoth to avoid issues during build
let mammothModule: any = null;

// Try to import mammoth, but handle it gracefully if it fails
const importMammoth = async () => {
  if (!mammothModule) {
    try {
      mammothModule = await import('mammoth');
    } catch (error) {
      console.error('Failed to import mammoth module:', error);
    }
  }
...
export const extractTextFromDocxBuffer = async (buffer: Buffer): Promise<string> => {
  try {
    ...
    if (!mammoth) {
      return '[Error: DOCX extraction module not available]';
    }
